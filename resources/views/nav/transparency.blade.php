...
@section('title', 'TESDA Transparency Portal')

@section('content')
<section class="max-w-6xl mx-auto px-6 py-12 text-gray-800">

    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-blue-800 mb-4">TESDA Occidental Mindoro Transparency Portal</h1>
        <p class="text-gray-700 max-w-3xl mx-auto">
            Access public documents, reports, and updates in line with transparency and accountability initiatives.
        </p>
    </div>

    <!-- ✅ Display Transparency Records -->
    <div class="overflow-x-auto">
        <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($transparencies as $item)
                    <tr>
                        <td class="px-6 py-3">{{ $item->title }}</td>
                        <td class="px-6 py-3">{{ Str::limit($item->description, 80) }}</td>
                        <td class="px-6 py-3">
                            @if($item->file)
                                <a href="{{ asset('storage/' . $item->file) }}" target="_blank" class="text-blue-600 underline hover:text-blue-800">
                                    View File
                                </a>
                            @else
                ...
                @empty
                    <tr>
                        <td colspan="3" class="px-6 py-3 text-gray-500 italic text-center">
                            No transparency records available yet.
                        </td>
                    </tr>
                @endforelse
            </tbody>
