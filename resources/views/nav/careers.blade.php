...
<section class="max-w-6xl mx-auto px-6 py-12 text-gray-800">
    <div class="text-center mb-12">
        <h1 class="text-4xl font-bold text-blue-800 mb-4">Careers at TESDA Occidental Mindoro</h1>
        <p class="text-gray-700 max-w-2xl mx-auto">
            Explore exciting opportunities to join our growing community of professionals.
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        @forelse ($careers as $career)
            <div class="bg-white rounded-lg shadow hover:shadow-lg transition p-6 text-center">
                @if($career->image)
                    <img src="{{ asset('storage/careers/' . $career->image) }}" 
                         alt="{{ $career->title }}" 
                         class="w-full h-48 object-cover rounded mb-4">
                @endif
                <h2 class="text-xl font-semibold mb-2 text-blue-700">{{ $career->title }}</h2>
                <p class="text-gray-600">{{ Str::limit($career->description, 120) }}</p>
            </div>
        @empty
